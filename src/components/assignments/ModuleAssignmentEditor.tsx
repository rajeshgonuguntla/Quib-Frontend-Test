...
interface CourseAssignmentEditorProps {
  courseId: string;
}

export function CourseAssignmentEditor({ courseId }: CourseAssignmentEditorProps) {
  ...
  const handleMaxPointsChange = (value: number) => {
    setMaxPoints(value);
    if (passingPoints > value) {
      setPassingPoints(value);
    }
  };

  const saveAssignment = async (fileOverride?: File | null) => {
    const fileToUpload = fileOverride ?? briefFile;
    if (!title.trim()) {
      setError('Title is required');
      return;
    }
    if (!assignment && !fileToUpload) {
      fileInputRef.current?.click();
      setError('Choose a PDF or TXT brief (max 5 MB), then click Create assignment again.');
      return;
    }
    if (passingPoints < 1 || passingPoints > maxPoints) {
      setError(`Passing marks must be between 1 and ${maxPoints}`);
      return;
    }
    setSaving(true);
    setError(null);
    setStatus(null);
    try {
      const saved = await upsertCourseAssignment(courseId, {
        title: title.trim(),
      ...
      setError(assignmentErrorMessage(err, 'Save failed'));
    } finally {
      setSaving(false);
    }
  };

  const handleSave = () => void saveAssignment();

  const handleBriefSelected = (file: File | null) => {
    if (!file) {
      setBriefFile(null);
      return;
    }
  ...
  const handleGrade = async (submissionId: string) => {
    ...
    setError(null);
    try {
      await gradeAssignmentSubmission(courseId, submissionId, points, draft?.feedback);
      setStatus(points >= passingPoints ? 'Marked as passed' : 'Returned to student for resubmission');
      await reload();
    } catch (err) {
  ...
  return (
    <div className="space-y-4 rounded-md border border-border p-4">
      ...
      <p className="text-xs text-muted-foreground">
        One assignment per course. Students pass when their score is at or above passing marks.
      </p>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="space-y-2">
          ...
          <Input type="number" min={1} max={1000} value={maxPoints} onChange={(e) => handleMaxPointsChange(Number(e.target.value))} />
        </div>
        <div className="space-y-2">
      ...
      <label className="flex items-center gap-2 text-sm">
        ...
        Required for course progress (student must pass to complete the course)
      </label>
      <label className="flex items-center gap-2 text-sm">
        <input
          type="checkbox"
      ...
      <div className="space-y-2">
        ...
        <input
          ...
          className="sr-only"
          onChange={(e) => {
            handleBriefSelected(e.target.files?.[0] ?? null);
          }}
        />
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <Upload size={14} />
            {briefFile ? 'Change file' : 'Choose brief file'}
          </Button>
          {briefFile && (
        ...
        {!assignment && !briefFile && (
          <p className="text-xs text-muted-foreground">
            Step 1: choose a PDF or TXT file. Step 2: click Create assignment.
          </p>
        )}
      </div>
      ...
      <Button
        ...
        disabled={saving || (!assignment && !title.trim())}
      >
            ...
            <div key={sub.submissionId} className="rounded-md border border-border p-3 space-y-2">
              <div className="flex flex-wrap justify-between gap-2 text-sm">
                ...
                <span className="text-muted-foreground">
                  {sub.status === 'graded' ? 'Passed' : sub.status === 'returned' ? 'Returned' : sub.status === 'submitted' ? 'Awaiting review' : sub.status}
                </span>
              </div>
              ...
              <Button
                ...
                size="sm"
                variant="outline"
                onClick={() => void downloadStudentSubmission(courseId, sub.submissionId, sub.filename)}
              >
                <Download size={12} /> Download
              </Button>
              {sub.status === 'submitted' ? (
                <div className="space-y-2">
                  <Input
                    placeholder={`Score / ${maxPoints} (pass ≥ ${passingPoints})`}
                  ...
                  />
                  <Button type="button" size="sm" onClick={() => void handleGrade(sub.submissionId)} disabled={saving}>
                    Submit grade
                  </Button>
                </div>
              ) : sub.status === 'graded' ? (
                <p className="text-sm">
                  Passed — <strong>{sub.gradePoints}</strong> / {sub.maxPoints}
                  {sub.educatorFeedback ? ` — ${sub.educatorFeedback}` : ''}
                </p>
              ) : sub.status === 'returned' ? (
                <p className="text-sm text-muted-foreground">
                  Returned for resubmission
                  {sub.gradePoints != null ? ` — score ${sub.gradePoints}/${sub.maxPoints}` : ''}
                  {sub.educatorFeedback ? ` — ${sub.educatorFeedback}` : ''}
                </p>
              ) : null}
            </div>
...
/** @deprecated Use CourseAssignmentEditor */
export const ModuleAssignmentEditor = CourseAssignmentEditor;
